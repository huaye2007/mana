...
class RpcNodeTest {
    ...
    void negativeCommandsSupportCallsAndOneWaySends() throws Exception {
        ...
        var b = node(20, (c, m) -> {
            var request = (RpcRequest) m;
            if (request.requestId() == 0) notice.complete(request.command());
            else {
                assertEquals(-1001, request.command());
                reply(request, body("reward"));
            }
        });
        connect(a, b, 1);
        var result = new TestSignal<RpcResult>();
        ...
        assertTrue(a.send(20, Integer.MIN_VALUE, body("notice"), RpcOptions.route(7)));
        assertEquals(Integer.MIN_VALUE, notice.get(3, TimeUnit.SECONDS));
        assertEquals(1, a.calls.requestIdCounter.get());
        assertThrows(IllegalArgumentException.class, () -> a.send(20, 0, body("zero"), RpcOptions.DEFAULT));
    }
