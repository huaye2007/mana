    ...
    static ByteBuf expected(String name) {
        return switch (name) {
            ...
            case "command_negative" ->
                    WIRE.request(-1001, 1, options(2, 3, (byte) 1, RpcMetadata.EMPTY), empty());
            case "command_min" ->
                    WIRE.request(Integer.MIN_VALUE, 0, options(2, 3, (byte) 1, RpcMetadata.EMPTY), empty());
            case "send_empty" ->
