...
final class RpcCalls {
    ...
    boolean isAccepting() { return accepting; }

    /** Claims pending calls under admission; never invokes user code or transport under the lock. */
    void stopCalls(java.util.function.Supplier<Collection<RpcPeer>> peers) {
        var closing = new LinkedHashMap<RpcPeer, List<RpcFuture>>();
        synchronized (admission) {
            if (!accepting) return;
            accepting = false;
            for (RpcPeer peer : peers.get()) closing.put(peer, settlePeer(peer));
        }
    ...
    RpcFuture begin(RpcPeer peer, long deadline, RpcCallback callback) {
        ...
        try {
            ...
            fail(peer, pending, localError(failure), failure);
        }
        return pending;
    }
    ...
    void fail(RpcPeer peer, RpcFuture pending, RpcError error) {
        fail(peer, pending, error, null);
    }

    void fail(RpcPeer peer, RpcFuture pending, RpcError error, Throwable cause) {
        finishCall(peer, pending, RpcResult.failure(error, cause), false);
    }
    ...
    void reject(int target, RpcCallback callback, RpcError error) {
        reject(target, callback, error, null);
    }

    void reject(int target, RpcCallback callback, RpcError error, Throwable cause) {
        observeCompletion(target, null, false, error);
        notifyResult(callback, RpcResult.failure(error, cause));
    }

    private RpcFuture register(RpcPeer peer, long deadline, RpcCallback callback) {
        synchronized (admission) {
            if (!accepting) throw RpcException.UNAVAILABLE;
            return registerAccepted(peer, deadline, callback);
        }
    }

    private RpcFuture registerAccepted(RpcPeer peer, long deadline, RpcCallback callback) {
