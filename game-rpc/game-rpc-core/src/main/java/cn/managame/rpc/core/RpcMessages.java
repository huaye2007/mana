...
import cn.managame.rpc.protocol.RpcMetadataUtil;
...
final class RpcMessages {
    ...
    private void validateRequest(int command, ByteBuf body, RpcOptions options) {
        if (command == 0) throw new IllegalArgumentException("command must not be zero");
    ...
    void call(int target, int command, ByteBuf body, RpcOptions options, RpcCallback callback) {
        ...
        if (!calls.isAccepting()) {
            calls.reject(target, callback, RpcError.UNAVAILABLE);
            return;
        }
        RpcPeer peer = null;
        ...
        try {
            ...
            if (pending != null) calls.fail(peer, pending, error, e);
            else calls.reject(target, callback, error, e);
        }
    }
    ...
    boolean send(int target, RpcMessage message, RpcOptions options) {
        ...
        if (!calls.isAccepting()) return false;
        try {
