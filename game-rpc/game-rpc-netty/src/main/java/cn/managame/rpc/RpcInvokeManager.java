...
import cn.managame.rpc.connection.RpcWriteCallback;
...
public class RpcInvokeManager {
    ...
    public boolean tryOneway(RpcConnection connection, RpcRequest request) {
        ...
        // fire-and-forget：不挂写回调（写失败也不重试），避免热路径每次分配 callback + listener
    ...
    public RpcFuture invoke(RpcConnection connection, RpcRequest request) {
        ...
        future.setTimeoutCancellation(timeout::cancel);

        try {
            connection.writeMsg(request, new RpcWriteCallback() {
                @Override
