...
Etcd 使用 `.type("etcd")`，`endpoints` 支持逗号分隔的地址。可选属性包括 `prefix`（默认 `/mana/services`）、`leaseTtlSeconds`（默认 10）、`username` 和 `password`。注册实例绑定到客户端 lease，关闭 registry 时会撤销 lease 并自动删除实例。
