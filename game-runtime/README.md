...
统一基于哈希时间轮（默认共享实例 tick 100ms × 512 槽），**禁止业务直接用 `ScheduledExecutorService`**。时间轮按单调钟绝对截止点调度，正常负载下绝不早于 deadline，最多晚一个 tick。取消会立即把 Timeout 推进到终态并更新 pendingCount；停机取消剩余 Timeout。`CronTask.start()` 只能调用一次，`cancel()` 会取消底层 Timeout。
