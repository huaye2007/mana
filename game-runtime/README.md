...
`RouteTask.observeCompletion(observer)` observes terminal success (`null`) or the original failure for infrastructure cleanup, including backend failures. Pending observers run on the completing thread; an observer registered after completion runs on the registering thread. They execute outside the completion lock, must return promptly, and have no Route/HandlerContext guarantee. Exceptions are isolated from task completion and other observers. Use `onComplete` for routed business notifications. RouteTask stores explicit terminal state and waits through a CountDownLatch; it exposes no arbitrary-thread continuation chain.

Adapters that already hold a route and metadata can call `runtime.callback(targetRoute, metadata, onSuccess, onFailure)`. Delivery uses the callback admission reserve. `runtime.dispatch(routeType, key, metadata, action)` similarly accepts explicit context data with ordinary task admission. Both overloads use the supplied metadata without inheriting current context or invoking MetadataPropagator. Existing overloads retain their context propagation behavior.
