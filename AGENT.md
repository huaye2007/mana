...
面向 AI 编码代理与贡献者的工作约定。项目背景、模块职责、常用入口见 [README.zh-CN.md](README.zh-CN.md)。
...
- 模块根目录的 `README.md`（英文）和 `README.zh-CN.md`（中文）都是面向外部使用者的公开文档；公开内容写 README，内部细节写 `docs/`。
- **公开 README 中不得出现指向 `docs/` 的链接**（该目录不上传，链接在公共仓库会 404）。
- **所有 README 必须中英双语两份**：`README.md` 为英文，`README.zh-CN.md` 为中文，两者内容保持同步，顶部互加语言切换链接（`[中文](README.zh-CN.md) | English` / `[English](README.md) | 中文`）。修改任一 README 时必须同步更新另一语言版本。
...
- 其余约定（运行时 routerKey 串行模型、依赖许可证要求等）见 [README.zh-CN.md](README.zh-CN.md) 的「开发约定」与「依赖说明」。
