 ...
 * metadata key 从 {@link Metadata#KEY_BUSINESS_MIN} 起，低段保留给 game-rpc 框架。</p>
 *
 ...
 * busType/busId=会话定位；metadata 带 {@link #MK_SEQ}（原样回echo），登录命令额外带
 * {@link #MK_CLIENT_IP}。</p>
 *
 * <p><b>下行</b>（后端→网关，oneway，走同一条连接回来）：command=回包/推送命令，body=回包体，
 * busType/busId=目标会话定位；metadata 带 {@link #MK_SEQ}（回请求的 seq，主动推送填 0）、
 * {@link #MK_CODE}（业务错误码）。后端首次绑定角色时在登录响应用 {@code routeKey} 回带 roleId，
 * 触发网关侧 roleId→会话 绑定与顶号。</p>
 */
public final class GatewayRpcProtocol {
    ...
    // 会话定位（sessionId / roleId）改走 busType+busId，故 KEY_BUSINESS_MIN+0/+1 两个 key 空出。

    /** 客户端 seq（long，实际取值在 int 范围）。上行原样带，下行回 echo；主动推送为 0。 */
    public static final short MK_SEQ = Metadata.KEY_BUSINESS_MIN + 2;         // 102

    /** 业务错误码（long，取值在 int 范围）。仅下行带，映射到外网帧 code。 */
    public static final short MK_CODE = Metadata.KEY_BUSINESS_MIN + 3;        // 103

    /** 客户端来源 IP（string）。仅上行登录命令带，供后端做风控/审计。 */
    public static final short MK_CLIENT_IP = Metadata.KEY_BUSINESS_MIN + 4;   // 104
