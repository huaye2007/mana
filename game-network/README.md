...
Connection.type() returns ConnectionType.TCP, WS, or WSS. The type is fixed at creation and does not change with handshake state. HTTP uses native handlers and does not create a Connection. NettyConnection holds a Channel and an immutable ConnectionType. id() directly returns a String from channel.id().asLongText(). State, remote address, attributes, and close delegate to native methods. write reserves per-connection outbound capacity before delegating to writeAndFlush; there is no separate message queue.

A false return from write means the Channel was inactive or its outbound admission budget was exhausted; the message was not taken over. A true return means the message was passed to the native write path; it does not mean the peer received it, nor does it guarantee atomic admission against a concurrent close. For portable completion notification, use `connection.write(message, failure -> { ... })`: null means local write success; otherwise the original failure is supplied. Netty implements this optional overload. A true return transfers ownership even on later failure; false retains caller ownership and invokes no callback. Notification may run before return or on the transport thread and must return promptly. A Connection without this optional capability throws UnsupportedOperationException before taking ownership. For direct access to a ChannelFuture, use NettyAccess.channel(connection).writeAndFlush(message). Native Channel writes bypass the connection admission budget. Choose only one write path to avoid transferring the same message reference twice.

TCP and WS builders accept `.outboundWriteLimits(new OutboundWriteLimits(1024, 8L * 1024 * 1024))` (`cn.managame.network.netty.connection.OutboundWriteLimits`); these are also the defaults. Both pending-write count and estimated bytes are reserved atomically on the calling thread, before any EventLoop task is queued, and returned when the native write completes, including failure. `isWritable()` also checks this capacity. A false write is final admission rejection: release the message, and decide whether the business protocol should drop, retry with its own bounded policy, or close a slow peer.

ByteBuf and ByteBufHolder use readable bytes; FileRegion uses remaining bytes. Other business objects use the native `ChannelOption.MESSAGE_SIZE_ESTIMATOR` configured on the builder (server: `childOption`, client: `option`). Supply an appropriate estimator for objects that encode to larger payloads, including expected framing overhead when appropriate. This is an admission estimate, not an exact memory cap after arbitrary user encoding/TLS expansion. The pending-write count limit applies to every object. Native HTTP handlers and direct Channel writes remain governed by the application's native pipeline.
