...
public final class NettyConnection implements Connection {
    ...
    @Override
    public boolean write(Object message, java.util.function.Consumer<? super Throwable> completion) {
        Objects.requireNonNull(message, "message");
        Objects.requireNonNull(completion, "completion");
        if (!channel.isActive()) return false;
        channel.writeAndFlush(message).addListener(result -> {
            try { completion.accept(result.isSuccess() ? null : result.cause()); }
